...
def test(args):
    ...
    if args.tto:
        ...
        model.load_state_dict(torch.load(os.path.join(args.weights_path, 'Best_Teacher_Model_{}_{}_{}.pkl'.format(args.teacher_type, args.dataset, args.num_classes))))
        print("Teacher Model is successfully loaded.")
    elif not args.kd:
        model = StudentModel(args.num_classes).to(device)
        model.load_state_dict(torch.load(os.path.join(args.weights_path, 'Best_Model_WithoutKD_{}_{}.pkl'.format(args.dataset, args.num_classes))))
        print("Model without Knowledge Distillation is successfully loaded.")
    elif args.kd:
        model = StudentModel(args.num_classes).to(device)
        model.load_state_dict(torch.load(os.path.join(args.weights_path, 'Best_Model_KD_{}_{}.pkl'.format(args.dataset, args.num_classes))))
