...
def main(args):
    ...
    if args.tto:
        ...
        teacher_model.load_state_dict(torch.load(os.path.join(args.weights_path, 'Best_Teacher_Model_{}_{}_{}.pkl'.format(args.teacher_type, args.dataset, args.num_classes))))
        model = StudentModel(args.num_classes).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_top1_accuracy > best_top1_accuracy:
            ...
            # Save Models #
            if args.tto:
                torch.save(model.state_dict(), os.path.join(args.weights_path, 'Best_Teacher_Model_{}_{}_{}.pkl'.format(args.teacher_type, args.dataset, args.num_classes)))
            elif not args.kd:
                torch.save(model.state_dict(), os.path.join(args.weights_path, 'Best_Model_WithoutKD_{}_{}.pkl'.format(args.dataset, args.num_classes)))
            elif args.kd:
                torch.save(model.state_dict(), os.path.join(args.weights_path, 'Best_Model_KD_{}_{}.pkl'.format(args.dataset, args.num_classes)))
